...
var once sync.Once
var manualRpcClient map[string]Client

func getManualRpcConfig(rkeys string) RpcClientConf {
	config := RpcClientConf{
...
func ManualConnection(rkeys string) Client {
	once.Do(func() {
		manualRpcClient = make(map[string]Client)
		manualRpcClient[rkeys] = MustNewClient(getManualRpcConfig(rkeys))
	})
	return manualRpcClient[rkeys]
}
