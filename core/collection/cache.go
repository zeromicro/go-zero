...
func NewBaseCache[T any](expire time.Duration, opts ...cacheOption[T]) (*cache[T], error) {
	cache := &cache[T]{
...
func WithBaseLimit[T any](limit int) cacheOption[T] {
...
func WithBaseName[T any](name string) cacheOption[T] {
	return func(cache *cache[T]) {
		cache.name = name
	}
}
