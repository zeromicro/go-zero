...
func NewPublisherEtcd(endpoints []string, key, value string, opts ...PublisherOption) Publisher {
	publisher := &PublisherEtcd{
		endpoints:  endpoints,
		key:        key,
		value:      value,
...
func (p *PublisherEtcd) KeepAlive() error {
...
func (p *PublisherEtcd) Pause() {
	p.pauseChan <- lang.Placeholder
}

func (p *PublisherEtcd) Resume() {
	p.resumeChan <- lang.Placeholder
}

func (p *PublisherEtcd) Stop() {
	p.quit.Close()
}

func (p *PublisherEtcd) keepAliveAsync(cli internal.EtcdClient) error {
...
func (p *PublisherEtcd) register(client internal.EtcdClient) (clientv3.LeaseID, error) {
...
func (p *PublisherEtcd) revoke(cli internal.EtcdClient) {
	if _, err := cli.Revoke(cli.Ctx(), p.lease); err != nil {
		logx.Error(err)
	}
}

func (p *PublisherEtcd) SetID(id int64) {
	p.id = id
}
