...
func TestPublisher_keepAliveAsyncWatchErrorRefreshesClient(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	firstWatchChan <- clientv3.WatchResponse{
		CompactRevision: 1,
	}
	...
	firstCli.EXPECT().Put(gomock.Any(), makeEtcdKey("thekey", int64(firstLease)), "thevalue", gomock.Any())
	firstCli.EXPECT().KeepAlive(gomock.Any(), firstLease).Return(firstKeepAliveChan, nil)
	firstCli.EXPECT().Watch(gomock.Any(), makeEtcdKey("thekey", int64(firstLease)), gomock.Any()).Return(
		(<-chan clientv3.WatchResponse)(firstWatchChan))
	firstCli.EXPECT().Revoke(gomock.Any(), firstLease)
	firstCli.EXPECT().Close()
	...
	secondCli.EXPECT().Watch(gomock.Any(), makeEtcdKey("thekey", int64(secondLease)), gomock.Any()).Return(
		(<-chan clientv3.WatchResponse)(secondWatchChan))
