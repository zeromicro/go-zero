...
type (
	SubscriberEtcd struct {
		items *container
	}
)

func NewSubscriberEtcd(endpoints []string, key string, opts ...SubOption) (Subscriber, error) {
...
func (s *SubscriberEtcd) AddListener(listener func()) {
	s.items.addListener(listener)
}

func (s *SubscriberEtcd) Values() []string {
