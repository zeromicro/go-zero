...
* 通过ddl生成
    ...
    ```shell script
    cztctl model mysql ddl -src="./*.sql" -dir="./sql/model" -c
    ```
...
* 通过datasource生成
    ...
    ```shell script
    cztctl model mysql datasource -url="user:password@tcp(127.0.0.1:3306)/database" -table="*"  -dir="./model"
    ```
...
* 带缓存模式
  * ddl
	...
	```shell script
	cztctl model mysql -src={patterns} -dir={dir} -cache
	```
  ...
  * datasource
	...
	```shell script
	cztctl model mysql datasource -url={datasource} -table={patterns}  -dir={dir} -cache=true
	```
...
* 不带缓存模式
  ...
  * ddl
      ...
      ```shell script
        cztctl model -src={patterns} -dir={dir}
      ```

  * datasource
  
      ```shell script
        cztctl model mysql datasource -url={datasource} -table={patterns}  -dir={dir}
      ```
