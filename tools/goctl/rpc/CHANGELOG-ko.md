...
`-I` / `--proto_path` 플래그로 외부 디렉터리의 proto 파일을 import할 수 있도록 지원을 추가했으며, 전체 전이 의존성 해석을 제공합니다.
...
- `parser/import.go` — 새 파일(대규모 추가). 재귀적 전이 import 해석을 위한 `ResolveImports()`, import된 proto에서 `go_package` / `package` 메타데이터를 추출하는 `ParseImportedProtos()`, proto package 이름으로 O(1) 조회를 제공하는 `BuildProtoPackageMap()`을 구현합니다.
...
| 전이 import(A → B → C) | ❌ 직접 import만 인식 | ✅ 모든 전이 의존성을 재귀적으로 해석 |
...
- `generator/typeref.go` — 새 파일. 핵심 타입 해석 엔진:
  - `resolveRPCTypeRef()` — proto RPC 타입(단순 타입, 동일 패키지 dotted 타입, 교차 패키지 dotted 타입, Google WKT)을 올바른 import 경로를 가진 Go 타입 참조로 해석합니다.
  - `resolveCallTypeRef()` — 타입 alias를 지원하는 클라이언트 코드 생성용 변형입니다.
  - `googleWKTTable` — 16개 Google well-known types 전체를 Go 대응 타입으로 매핑하는 테이블입니다.
- `generator/genserver.go` — `genFunctions()`가 이제 요청/응답 타입에 대해 `resolveRPCTypeRef()`를 호출하고 추가 import 경로를 수집합니다.
- `generator/genlogic.go` — `genLogicFunction()`이 `resolveRPCTypeRef()`를 사용합니다. main pb import와 교차 패키지 import를 조건부로 포함하는 `addLogicImports()`를 추가했습니다.
...
- `generator/call.tpl` — 교차 패키지 import 라인을 위한 `{{.extraImports}}` placeholder를 추가했습니다.
...
| `ext.ExtReq`(같은 `go_package`) | ❌ 오류: "request type must defined in" | ✅ `pb.ExtReq` — 메인 패키지에 병합 |
...
- 동일 패키지 dotted 타입(예: `ext.ExtReq`, `ext`가 같은 `go_package`를 가진 경우)은 `pb.ExtReq`로 해석됩니다.
- 교차 패키지 dotted 타입(예: `common.TypesReq`, `common`이 다른 `go_package`를 가진 경우)은 올바른 Go import 경로가 자동 추가된 `common.TypesReq`로 해석됩니다.
...
**영향받는 파일:**
- `generator/typeref.go` — `resolveGoogleWKT()` + `googleWKTTable`이 모든 표준 타입을 처리합니다.

**이전 vs 이후:**

| Proto Type | 이전(RPC param) | 이후(RPC param) |
|---|---|---|
...
#### RPC 정의에서 Dotted 타입 이름 허용

이전에는 goctl이 dot이 포함된 RPC 요청/응답 타입(예: `base.Req`)을 거부했으며, 모든 타입이 같은 proto 파일에 정의되어 있어야 했습니다. 이 제한이 제거되었습니다.

**이전 vs 이후:**

| Proto Definition | 이전 | 이후 |
|---|---|---|
...
**영향받는 파일:**
- `parser/service.go` — dot이 포함된 타입 이름을 `"request type must defined in"` / `"returns type must defined in"` 오류로 거부하던 검증 루프를 제거했습니다.
- `parser/parser_test.go` — `TestDefaultProtoParseCaseInvalidRequestType`과 `TestDefaultProtoParseCaseInvalidResponseType`을 이름 변경 및 업데이트하여 dotted 타입이 이제 성공적으로 파싱되는지 검증합니다.
