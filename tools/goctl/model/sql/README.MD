...
* 默认规则
  ...
  ，而updateTime支持`ON UPDATE CURRENT_TIMESTAMP`，对于这两个字段生成`insert`、`update`
...
* 带缓存模式
  ...
  代码会生成带缓存逻辑的代码，目前仅支持单索引字段（除全文索引外），对于联合索引我们默认认为不需要带缓存，且不属于通用型代码，因此没有放在代码生成行列，如example中user表中的`id`
  、`name`、`mobile`字段均属于单字段索引。
...
参考自 [go-sql-driver](https://github.com/go-sql-driver/mysql/blob/master/fields.go#L140)

| mysql dataType | golang dataType(unsigned/signed) | golang Null dataType |
|----------------|----------------------------------|----------------------|
| bool           | bool/bool                        | sql.NullBool         |
| boolean        | bool/bool                        | sql.NullBool        |
| tinyint        | int8/uint8                       | sql.NullInt64        |
| smallint       | int16/uint16                     | sql.NullInt64        |
| year           | int16/uint16                     | sql.NullInt64        |
| mediumint      | int32/uint32                     | sql.NullInt64        |
| int            | int32/uint32                     | sql.NullInt64        |
| integer        | int32/uint32                     | sql.NullInt64        |
| bigint         | int64/uint64                     | sql.NullInt64        |
| float          | float32/float32                  | sql.NullFloat64      |
| double         | float64/float64                  | sql.NullFloat64      |
| date           | time.Time/time.Time              | sql.NullTime         |
| datetime       | time.Time/time.Time              | sql.NullTime         |
| timestamp      | time.Time/time.Time              | sql.NullTime         |
| char           | string/string                    | sql.NullString       |
| varchar        | string/string                    | sql.NullString       |
| binary         | string/string                    | sql.NullString       |
| varbinary      | string/string                    | sql.NullString       |
| bit            | sql.RawBytes/sql.RawBytes        | sql.RawBytes         |
| enum           | sql.RawBytes/sql.RawBytes        | sql.RawBytes         |
| set            | sql.RawBytes/sql.RawBytes        | sql.RawBytes         |
| tniyblob       | sql.RawBytes/sql.RawBytes        | sql.RawBytes         |
| mediumblob     | sql.RawBytes/sql.RawBytes        | sql.RawBytes         |
| longblob       | sql.RawBytes/sql.RawBytes        | sql.RawBytes         |
| geometry       | sql.RawBytes/sql.RawBytes        | sql.RawBytes         |
| json           | sql.RawBytes/sql.RawBytes        | sql.RawBytes         |
| time           | sql.RawBytes/sql.RawBytes        | sql.RawBytes         |
| decimal        | sql.RawBytes/sql.RawBytes        | sql.RawBytes         |
| tinytext       | sql.RawBytes/sql.RawBytes        | sql.RawBytes       |
| text           | sql.RawBytes/sql.RawBytes        | sql.RawBytes       |
| mediumtext     | sql.RawBytes/sql.RawBytes        | sql.RawBytes       |
| fixed          | sql.RawBytes/sql.RawBytes        | sql.RawBytes       |
| real           | sql.RawBytes/sql.RawBytes        | sql.RawBytes       |